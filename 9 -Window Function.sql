...
SELECT constituency,party, votes,RANK() over (PARTITION BY constituency ORDER by votes desc) as rnk
  FROM ge
...
from (SELECT party,constituency, RANK() over (PARTITION by constituency ORDER BY votes desc) as rnk from ge WHERE constituency BETWEEN 'S14000021' AND 'S14000026'
   AND yr  = 2017) as a
where a.rnk  = 1
