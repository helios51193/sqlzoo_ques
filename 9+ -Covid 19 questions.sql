...
SELECT 
   ...
   RANK() OVER (ORDER BY confirmed/population ) as rnk
...
/* DISCLAMAIR - TODO */
select name,date,max(newcase) peak
from
(select name,
       DATE_FORMAT(whn,'%Y-%m-%d') date,
      (confirmed-LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn)) newcase
from covid) temp
where temp.newcase >= 1000
group by name
order by date 
